...
class EpochSummaryLogger(Callback):
    ...
    def _get_batch_size(self, batch) -> int:
        ...
        try:
            ...
            if isinstance(batch, (list, tuple)) and len(batch) > 0:
                ...
                if isinstance(first, (list, tuple)) and hasattr(first[0], "shape"):
    ...
    def on_train_batch_end(self, trainer: Trainer, pl_module: LightningModule, outputs, batch, batch_idx: int, dataloader_idx: int = 0) -> None:
        if outputs is None:
            return
    ...
    def on_validation_batch_end(self, trainer: Trainer, pl_module: LightningModule, outputs, batch, batch_idx: int, dataloader_idx: int = 0) -> None:
        if outputs is None:
            return
        loss = outputs.get("loss") if isinstance(outputs, dict) else getattr(outputs, "loss", None)
        if loss is None:
    ...
    def on_validation_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        ...
        val_avg = self.val_loss_sum / self.val_samples if self.val_samples > 0 else float("nan")
        ...
        for p in pl_module.parameters():
            ...
            total += num
            # near-zero check
            zeros += int(((p.abs() <= self.sparsity_threshold).sum().item()))
