...
def _process_vox_wav(task_tuple: Tuple[str, Path, pd.DataFrame, str, float]):
    ...
    assert speaker_id.startswith(
        dataset_name + "_id"
    ), f"Unexpected speaker ID format: {speaker_id}"
...
class VoxCelebProcessor:
    ...
    def __init__(
        ...
        self.downloaded_metadata_dir = (
            self.root_dir / self.DATASET_PATHS["downloaded_metadata_dir"]
        )

        # Downloaded metadata files
        self.vox1_metadata = self.downloaded_metadata_dir / os.path.basename(
            self.METADATA_URLS["vox1"]
        )
        self.vox2_metadata = self.downloaded_metadata_dir / os.path.basename(
            self.METADATA_URLS["vox2"]
        )

        # Created files - now generic (not test-specific)
        self.vox_metadata = (
            self.artifcats_dir / self.DATASET_PATHS["vox_metadata"]
        )
        self.preprocess_stats_file = (
            self.artifcats_dir / self.DATASET_PATHS["preprocess_stats_file"]
        )
        ...
        for dataset, url in self.METADATA_URLS.items():
            ...
            if not target_path.exists():
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to download metadata for {dataset}: {e}"
                    )
    ...
    def _create_combined_speaker_metadata(
        self,
    ) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, int]]:
        """Load and merge speaker metadata from both VoxCeleb1 and
        VoxCeleb2."""
    ...
    def _get_voxceleb_utterances(
        self, wav_paths: List[str], min_duration: float
    ) -> Tuple[List["VoxCelebUtterance"], Dict]:
        """Process WAV files in parallel using ProcessPoolExecutor (clean &
        fast)."""
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            ...
            future_to_path = {
                executor.submit(_process_vox_wav, task): task[0]
                for task in tasks
            }
        ...
        if self.verbose:
            log.info(
                f"Processing complete: {valid_count:,} valid files, {rejected_count:,} rejected"
            )
    ...
    def generate_metadata(
        ...
        utterances, utterances_stats = self._get_voxceleb_utterances(
            wav_paths, min_duration
        )
        utterances_stats = pd.DataFrame.from_dict(
            utterances_stats, orient="columns"
        )
        ...
        vad = (
            instantiate(vad_cfg)
            if vad_cfg and getattr(vad_cfg, "_target_", None)
            else None
        )
        if vad is not None and vad.should_apply("dev"):
            ...
            vad_rows = vad.apply(
                ...
                skip_list_path=self.artifcats_dir / "vad_skipped_dev.txt",
            )
        ...
        if self.use_pre_segmentation:
            if self.verbose:
                log.info(
                    f"Pre-segmentation enabled. Processing {len(dev_metadata)} utterances..."
                )
            ...
            for _, row in tqdm(
                ...
                segments = segment_utterance(
                    ...
                    vad_speech_timestamps=row.get(
                        "vad_speech_timestamps", None
                    ),
                    segment_max_silence_ratio=float(
                        getattr(vad, "segment_max_silence_ratio", 0.80)
                    )
            ...
            skipped_pct = (
                (skipped_utterances / total_utterances) * 100
                if total_utterances > 0
                else 0.0
            )
            avg_segment_length = sum(
                s.get("segment_duration", 0.0) for s in all_segments
            ) / len(all_segments)

            log.info(
                f"Generated {len(all_segments)} segments from {total_utterances} utterances"
            )
            log.info(
                f"Average segments per utterance: {len(all_segments)/total_utterances:.2f}"
            )
            log.info(f"Average segment duration: {avg_segment_length:.3f}s")
            log.info(
                f"Skipped {skipped_utterances} utterances ({skipped_pct:.2f}%) due to duration < min_segment_duration={self.min_segment_duration}s or silence filtering"
            )
        ...
        if save_df:
            VoxCelebProcessor.save_csv(
                dev_metadata, self.dev_metadata_file, sep=self.sep
            )
            VoxCelebProcessor.save_csv(
                utterances_stats, self.preprocess_stats_file, sep=self.sep
            )
            VoxCelebProcessor.save_csv(
                self.speaker_metadata_df, self.vox_metadata, sep=self.sep
            )
            if self.verbose:
                log.info(
    ...
    def utterances_to_csv(
        self,
        utterances: List[VoxCelebUtterance],
        dev_metadata_file: Union[str, Path],
    ) -> None:
        """Save list of VoxCelebUtterance objects to CSV file with pipe
        delimiter.
    ...
    def enrich_verification_file(
        ...
        if veri_df is None:
            if veri_test_path is None:
                raise ValueError(
                    "Either veri_test_path or veri_df must be provided"
                )
            veri_df = pd.read_csv(
                veri_test_path,
                sep=" ",
            ...
            if "enrollment" in veri_df.columns:
                veri_df.rename(
                    columns={"enrollment": "enroll_path", "test": "test_path"},
                    inplace=True,
                )
        ...
        veri_df["enroll_id"] = (
            dataset_prefix + veri_df["enroll_path"].str.split("/").str[0]
        )
        veri_df["test_id"] = (
            dataset_prefix + veri_df["test_path"].str.split("/").str[0]
        )
        ...
        for field in [DATESET_CLS.NATIONALITY, DATESET_CLS.GENDER]:
            veri_df[f"enroll_{field}"] = veri_df["enroll_id"].apply(
                lambda x: metadata_indexed.loc[x, field]
                if x in metadata_indexed.index
                else "N/A"
            )
        ...
        veri_df[f"same_{DATESET_CLS.NATIONALITY}"] = (
            (
                veri_df[f"enroll_{DATESET_CLS.NATIONALITY}"]
                == veri_df[f"test_{DATESET_CLS.NATIONALITY}"]
            )
            & (veri_df[f"enroll_{DATESET_CLS.NATIONALITY}"] != "N/A")
        ).astype(int)

        veri_df[f"same_{DATESET_CLS.GENDER}"] = (
            veri_df[f"enroll_{DATESET_CLS.GENDER}"]
            == veri_df[f"test_{DATESET_CLS.GENDER}"]
        ).astype(int)
        ...
        column_order = [
            ...
            "enroll_id",
            "test_id",
            f"enroll_{DATESET_CLS.GENDER}",
            f"test_{DATESET_CLS.GENDER}",
            f"enroll_{DATESET_CLS.NATIONALITY}",
            f"test_{DATESET_CLS.NATIONALITY}",
            f"same_{DATESET_CLS.GENDER}",
            f"same_{DATESET_CLS.NATIONALITY}",
        ]
    ...
    def print_test_statistics(df: pd.DataFrame) -> None:
        ...
        log.info(
            f"Same nationality trials: {df[f'same_{DATESET_CLS.NATIONALITY}'].sum()}"
        )
...
class VoxCelebTestFilter:
    ...
    def __init__(self, root_dir: Union[str, Path], verbose: bool = True):
        ...
        self.downloaded_metadata_dir = (
            self.root_dir
            / VoxCelebProcessor.DATASET_PATHS["downloaded_metadata_dir"]
        )
        self.verbose = verbose
    ...
    def download_test_file(self, test_file: str) -> Path:
        ...
        if test_file not in test_urls:
            raise ValueError(
                f"Unsupported test file: {test_file}. Supported options: {list(test_urls.keys())}"
            )
        ...
        if not target_path.exists():
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to download test file {test_file} from {url}: {e}"
                )
        else:
            if self.verbose:
                log.info(
                    f"Test file {test_file} already exists at: {target_path}"
                )
    ...
    def get_test_speakers(self, test_file: str) -> Tuple[set, pd.DataFrame]:
        ...
        veri_df = pd.read_csv(
            ...
            sep=" ",
            header=None,
            names=["label", "enrollment", "test"],
        )

        enrollment_spks = set(
            veri_df.enrollment.apply(lambda x: x.split(os.sep)[0])
        )
        test_spks = set(veri_df.test.apply(lambda x: x.split(os.sep)[0]))
        assert (
            test_spks == enrollment_spks
        ), "Enrollment and test speakers don't match"
    ...
    def filter_dev_metadata(
        self, dev_metadata_df: pd.DataFrame, test_speakers: set
    ) -> pd.DataFrame:
        """Filter out test speakers from development metadata."""
        # Extract speaker ID without dataset prefix for comparison
        dev_metadata_df["raw_speaker_id"] = dev_metadata_df[
            DATESET_CLS.SPEAKER_ID
        ].apply(lambda x: x.replace(DATASET_DEFAULTS.dataset_name + "_", ""))

        before_count = len(dev_metadata_df)
        filtered_df = dev_metadata_df[
            ~dev_metadata_df["raw_speaker_id"].isin(test_speakers)
        ].copy()
        ...
        if self.verbose:
            log.info(
                f"Filtered out {excluded_count} utterances from {len(test_speakers)} test speakers"
            )
            log.info(
                f"Remaining utterances: {after_count} (from {before_count})"
            )
...
if __name__ == "__main__":
    ...
    config.artifacts_dir = (
        f"{data_dir}/voxceleb/voxceleb_metadata/{artifacts_subdir}"
    )
    ...
    if not resolved_root.is_dir():
        raise NotADirectoryError(
            f"Root directory is not a directory: {resolved_root}"
        )
    ...
    dev_metadata = pd.read_csv(
        str(voxceleb_processor.dev_metadata_file.resolve()), sep=config.sep
    )
    ...
    if predict_lang:
        ...
        lang_id_model = LanguagePredictionModel(
            wav_dir=voxceleb_processor.wav_dir, crop_len=8
        )
    ...
    for test_file in veri_test_filenames:
        test_filter = VoxCelebTestFilter(
            root_dir=resolved_root, verbose=config.verbose
        )
        ...
        VoxCelebProcessor.save_csv(
            filtered_dev_metadata, filtered_dev_file, sep=config.sep
        )
    ...
    base_artifacts = (
        resolved_artifacts.parent
        if resolved_artifacts.name in veri_test_filenames
        else resolved_artifacts
    )

    # Build snapshot by extracting only the comparable keys from config
    snapshot_source = {
        key: getattr(config, key)
        for key in VOXCELEB_COMPARABLE_KEYS
        if hasattr(config, key)
    }
    snapshot = build_config_snapshot_from_mapping(
        snapshot_source, VOXCELEB_COMPARABLE_KEYS
    )
