...
def _process_experiment(
    ...
    *,
    ...
    data = read_pruning_experiment(
        exp_dir, info, include_all_layers=include_all_layers,
    )
    if data is None:
        return None
    ...
    if layers_enriched:
        ...
        render_layerwise_sparsity(
            layers_enriched, ax, annotate=annotate_layerwise,
        )
        ...
        fig.suptitle(
            title
            + (r"  —  per-row (blue) / per-column (red) density"),
            fontsize=9,
        )
        fig.supxlabel(
            "Per-row / per-column density (1 = retained)", fontsize=8,
        )
        ...
        render_mask_heatmaps(
            layers_enriched, axes, max_heatmap_panels,
        )
    ...
    if rtf_args is not None:
        rtf = read_rtf(
            exp_dir, data["ckpt"], layers_enriched,
...
def _emit_overlay_rates(
    ...
    *,
    ...
    panel = render_layerwise_panel(
        ...
        target_rates=sorted(
            {s["info"]["sparsity"] for s in members}
        ),
    ...
    emit_layerwise_legend(
        fig, out_path, handles, labels, legend_mode=legend_mode,
    )
...
def _emit_per_rate_cross_model(
    ...
    *,
    ...
    for rate, members in sorted(groups.items()):
        ...
        ylim_lo, ylim_hi = (
            ylim if ylim is not None
            else ylim_for_rate(rate, scale="percent")
        )
        axes[0].set_ylim(ylim_lo, ylim_hi)
        _layerwise_axis_labels(axes, twins)

        target_label = (
            rf"$\mathsf{{s}}^*$={rate}\%" if usetex
            else f"s={rate}%"
        )
        bar_label = (
            (r"\# parameters" if usetex else "# parameters")
            if bar_handle else None
        )
        handles, labels = assemble_layerwise_legend(
            all_handles,
        ...
        out_path = os.path.join(
            out_dir, f"cross_model_layerwise_sr{rate}.pdf"
        )
        emit_layerwise_legend(
            fig, out_path, handles, labels, legend_mode=legend_mode,
        )
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        choices=["overlay-rates", "per-rate-cross-model", "no-bucket",
                 "all"],
        default="all",
    )
    ap.add_argument(
    ...
    if not summaries:
        log.error(
            "No experiments produced summaries — nothing to aggregate."
        )
        return 1
    ...
    fig.savefig(
        os.path.join(
            cross_dir, "structural_density_vs_sparsity.pdf",
        ),
    )
    plt.close(fig)
    ...
    if "overlay-rates" in modes:
        _emit_overlay_rates(
            summaries,
            os.path.join(
                cross_dir, "layerwise_sparsity_all_rates.pdf",
            ),
            **common_kw,
        )
