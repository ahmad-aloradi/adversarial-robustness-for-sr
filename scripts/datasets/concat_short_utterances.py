#!/usr/bin/env python3
"""Concatenate short utterances from the same speaker AND genre to create
longer audio files.
...
def scan_audio_file(args: Tuple[str, str, str]) -> Optional[AudioFileInfo]:
    ...
    if not speaker_id or not speaker_id.startswith("id"):
        raise ValidationError(
            f"Could not extract speaker ID from path: {rel_path}"
        )
    ...
    if "-" in filename_stem:
        ...
        raise ValidationError(
            f"Could not extract genre from filename: {filename}"
        )
...
def scan_dataset(
    ...
    for sub_dataset in sub_datasets:
        ...
        if not sub_root.exists():
            raise ValidationError(
                f"Sub-dataset directory does not exist: {sub_root}"
            )
        ...
        if not audio_files:
            raise ValidationError(
                f"WARNING: No {extensions} files found in {data_dir}"
            )
        else:
            print(f"Found {len(audio_files)} audio files in {data_dir}")

        all_files.extend(
            (str(f), str(sub_root), sub_dataset) for f in audio_files
        )
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        futures = {
            executor.submit(scan_audio_file, task): task for task in all_files
        }
        for future in tqdm(
            as_completed(futures), total=len(futures), desc="Scanning files"
        ):
            try:
                result = future.result()
            except Exception as e:
                raise RuntimeError(
                    f"ERROR: Failed to scan file {futures[future]}: {e}"
                ) from e
            if result is not None:
                results.append(result)

    # Sort for deterministic ordering (parallel execution order is arbitrary)
    results.sort(key=lambda f: (f.sub_dataset, f.rel_path))
    return results
...
def group_short_files_by_speaker_and_genre(
    ...
    """Group files by (speaker_id, genre), separating short files from long
    files.
    ...
    short_by_speaker_genre: Dict[
        Tuple[str, str], List[AudioFileInfo]
    ] = defaultdict(list)
...
def create_concat_groups(
    ...
    if not short_by_speaker_genre:
        raise ValidationError(
            "Cannot create groups from empty speaker-genre dictionary"
        )

    # Use min_threshold if provided, otherwise fall back to target_duration
    effective_min = (
        min_threshold if min_threshold is not None else target_duration
    )
    ...
    for (speaker_id, genre), files in sorted(short_by_speaker_genre.items()):
        if not files:
            continue

        # Sort by duration descending, with rel_path tiebreaker for determinism
        sorted_files = sorted(files, key=lambda x: (-x.duration, x.rel_path))
        ...
        for file_info in sorted_files:
            ...
            if (
                current_group.total_duration >= target_duration
                and len(current_group.source_files) > 1
            ):
        ...
        if current_group.source_files:
            if speaker_genre_groups:
                ...
                speaker_genre_groups[-1].source_files.extend(
                    current_group.source_files
                )
                speaker_genre_groups[
                    -1
                ].total_duration += current_group.total_duration
            elif (
                len(current_group.source_files) > 1
                and current_group.total_duration >= effective_min
            ):
                # No previous groups, but meets min_threshold with multiple files
                speaker_genre_groups.append(current_group)
                if (
                    min_threshold is not None
                    and current_group.total_duration < target_duration
                ):
    ...
    all_groups.sort(key=lambda g: (g.speaker_id, g.genre))
    ...
    if groups_below_target_but_included:
        ...
        if len(groups_below_target_but_included) > 5:
            print(
                f"  ... and {len(groups_below_target_but_included) - 5} more"
            )
        print()
...
def concatenate_audio_files(
    ...
    speaker_ids = {f.speaker_id for f in group.source_files}
    ...
    genres = {f.genre for f in group.source_files}
    if len(genres) > 1:
        raise ValidationError(
    ...
    # Create output path: output_dir/{speaker_id}/{genre}/concat_NNN{extensions}
    # group_idx is a per-(speaker_id, genre) counter for meaningful naming
    speaker_genre_dir = output_dir / group.speaker_id / group.genre
    speaker_genre_dir.mkdir(parents=True, exist_ok=True)

    output_filename = f"concat_{group_idx:04d}{extensions}"
    ...
    assert (
        duration_diff < 0.01
    ), f"Significant duration mismatch for {output_path}: "

    # Compute relative paths for CSV
    output_rel_path = str(output_path.relative_to(root_dir))
    source_rel_paths = [
        str(Path(f.sub_dataset) / f.rel_path) for f in included_files
    ]
...
def main():
    ...
    if args.target_duration <= 0:
        raise ValidationError(
            f"Invalid target_duration: {args.target_duration}"
        )
    ...
    if min_threshold is not None:
        print(
            f"Min threshold: {min_threshold}s (include groups below target if >= this)"
        )
    ...
    (
        short_by_speaker_genre,
        long_files,
    ) = group_short_files_by_speaker_and_genre(all_files, args.target_duration)

    total_short = sum(len(files) for files in short_by_speaker_genre.values())
    unique_speakers = len({k[0] for k in short_by_speaker_genre.keys()})
    unique_genres = len({k[1] for k in short_by_speaker_genre.keys()})
    print(
        f"Found {total_short} short files (< {args.target_duration}s) "
        f"across {len(short_by_speaker_genre)} (speaker, genre) groups"
    )
    print(
        f"  Unique speakers: {unique_speakers}, Unique genres: {unique_genres}"
    )
    ...
    for group in tqdm(concat_groups, desc="Concatenating"):
        key = (group.speaker_id, group.genre)
        group_idx = group_counters[key]
        ...
        (
            ...
            group, root_dir, output_dir, group_idx, extensions
        )
    ...
    assert len(verify_df) == len(
        mapping_df
    ), "Mismatch in saved mapping file rows"

    # Calculate final statistics
    total_merged = sum(
        len(row["source_paths"].split(";")) for row in mapping_rows
    )
    ...
    print(
        "2. The preprocessing will skip merged source files and include concatenated files"
    )
