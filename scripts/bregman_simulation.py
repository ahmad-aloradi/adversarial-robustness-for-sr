...
across the two λ-change rescale modes: nesterov_adaptive_update and
...
def prox_l1(x, delta, lamda):
    ...
    return (
        np.sign(x) * np.maximum(np.abs(x) - delta * lamda, 0.0) / (1 + delta)
    )
...
def run(optimizer, mode, w_init, all_grads):
    """
    optimizer: 'linbreg' or 'adabreg'
    mode: 'nesterov' or 'subgrad_correction'
    """
    w = w_init.copy()
    lamda = (
        lambda_init_linbreg if optimizer == "linbreg" else lambda_init_adabreg
    )
    prev_lamda = lamda
    ...
    for t in range(T):
        ...
        # Subgradient correction (before the dual step): patch the dual when λ
        # changes so v stays a valid subgradient of the new R.
...
# LinBreg/AdaBreg × {nesterov, subgrad_correction}.
configs = OrderedDict()
for opt in ["linbreg", "adabreg"]:
    for mode in ["nesterov", "subgrad_correction"]:
        label = f"{opt}/{mode}"
...
STYLES = {
    "linbreg/nesterov": {
        "color": "#7991a9",
        "ls": "--",
        "label": "LinBreg/nesterov",
    },
    "linbreg/subgrad_correction": {
        "color": "#022547",
        "ls": ":",
        "label": "LinBreg/subgrad",
    },
    "adabreg/nesterov": {
        "color": "#b2182b",
        "ls": "--",
        "label": "AdaBreg/nesterov",
    },
    "adabreg/subgrad_correction": {
        "color": "#54030d",
        "ls": ":",
        "label": "AdaBreg/subgrad",
    },
}
...
ax.axhline(
    target_sparsity, color="gray", linestyle="--", alpha=0.5, linewidth=0.8
)
ax.set_xlabel("Step")
...
fig2, axes2 = plt.subplots(
    len(dist_configs), len(dist_steps), figsize=(18, 12)
)
fig2.suptitle(
    "Weight Distributions at Key Steps", fontsize=13, fontweight="bold"
)
...
plt.tight_layout()
plt.savefig(
    "results/bregman_simulation_distributions.png",
    dpi=150,
    bbox_inches="tight",
)
plt.close()
...
print("=" * 110)
print(
    f"N={N}, T={T}, δ={delta}, lr (LinBreg)={lr_linbreg}, lr (AdaBreg)={lr_adabreg}, σ_w={sigma_w}, σ_grad={sigma_grad}"
)
print(
    f"λ₀ LinBreg={lambda_init_linbreg}, λ₀ AdaBreg={lambda_init_adabreg}, target={target_sparsity}, accel={acceleration_factor}"
)
print()
...
for name, h in configs.items():
    ...
    first_reach = next(
        (
            t
            for t, s in enumerate(h["sparsity"])
            if s >= target_sparsity - 0.01
        ),
        None,
    )
    ...
    print(
        f"  {STYLES[name]['label']:.<30s} sparsity={final_s:.4f}  λ={final_l:.4f}  "
