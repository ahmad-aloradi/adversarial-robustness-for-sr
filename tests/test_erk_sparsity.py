...
from src.callbacks.pruning.utils.erk_sparsity import (
    ERK_BUDGET_TOL,
    LayerShape,
    erk_initial_and_target_sparsity,
    raw_density,
    solve_erk_densities,
)
...
# =============================================================================
# 5. initial vs target sparsity
# =============================================================================


def test_initial_defaults_to_target():
    """initial_target_sparsity=None => each layer starts AT its target."""
    shapes = [
        LayerShape(
            "a",
            fan_in=64,
    ...
    out = erk_initial_and_target_sparsity(shapes, 0.9, "erk", None)
    for name in ("a", "b"):
        assert out[name]["initial_sparsity"] == out[name]["target_sparsity"]


def test_denser_initial_start():
    """A denser initial budget makes every layer start sparser than its
    target."""
    shapes = [
        LayerShape(
            "a",
    ...
    out = erk_initial_and_target_sparsity(shapes, 0.9, "erk", 0.99)
    for name in ("a", "b"):
        assert out[name]["initial_sparsity"] > out[name]["target_sparsity"]
...
# =============================================================================
# 7. Degenerate target: 0.0 keeps everything dense
# =============================================================================
